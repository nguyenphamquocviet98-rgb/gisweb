...
st.set_page_config(
    layout="wide",
    page_title="Hệ Thống Phân Tích Vệ Tinh Pro",
    page_icon="🛰️",
...
# =========================================================
# 3. CSS GIAO DIỆN - HIỆN ĐẠI & CHUYÊN NGHIỆP
# =========================================================
st.markdown("""
<style>
:root {
...
def get_provinces(country_name: str) -> List[str]:
    try:
        ...
        st.error(f"Lỗi tải tỉnh/bang: {e}")
        return []
...
def search_best_images(country: str, provinces: Tuple[str, ...], year: str, month: str, layer: str) -> Dict[str, Optional[str]]:
    try:
        ...
        return {"Lỗi tải ảnh - dùng mặc định": None}
...
def build_report_html(params: AnalysisParams, result: dict) -> str:
    ...
    html = f"<div class='report-card'><strong style='color:#0066cc; font-size:15px'>📍 KHU VỰC: {result['roi_names']}</strong><br><br>"

    if first_y == last_y:
        html += f"<strong style='color:#667eea;'>1. TÓM TẮT HIỆN TRẠNG ({last_y})</strong><br><br>"
        if l_mean == 0:
            ...
            html += (
                ...
                f"<b style='color:#d32f2f; font-size:16px;'>{mean_text}</b>.<br>"
        ...
        html += f"<strong style='color:#667eea;'>1. PHÂN TÍCH BIẾN ĐỘNG ({first_y} ➔ {last_y})</strong><br><br>"

        if a_first_high == 0 and a_last_high == 0:
            html += "<span style='color:#ff9800'>⚠️ Mây che phủ diện rộng, không thể nội suy diện tích.</span>"
        else:
            ...
            color = "#d32f2f" if diff_ha > 0 else "#00a86b"
            if params.layer == "NDVI":
                color = "#00a86b" if diff_ha > 0 else "#d32f2f"
...
# =========================================================
# 12. GIAO DIỆN CHÍNH - HIỆN ĐẠI & CHUYÊN NGHIỆP
# =========================================================
# Hiển thị tiêu đề chính
st.markdown("""
<div class="main-title">
    🛰️ HỆ THỐNG PHÂN TÍCH VỆ TINH PRO
    <span style="font-size: 14px; display: block; margin-top: 5px; opacity: 0.9;">
        Phân tích đa thời gian với Google Earth Engine
    </span>
</div>
""", unsafe_allow_html=True)
...
with col_control:
    with st.container(height=900, border=False):
        ...
        with st.container(border=True):
            st.markdown("<div class='box-title'>1️⃣ Chọn khu vực</div>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            st.markdown("<div class='box-title'>2️⃣ Thông số ảnh</div>", unsafe_allow_html=True)
            ...
            c1, c2 = st.columns(2)
            year_base = c1.selectbox(
                "📅 Năm gốc",
                CONFIG["years"],
                index=CONFIG["years"].index("2023"),
                label_visibility="collapsed"
            )
            month = c2.selectbox(
                "🔢 Tháng",
            ...
            if st.button("🔎 Tải ảnh ít mây", use_container_width=True):
                if not selected_provinces:
                    st.warning("⚠️ Vui lòng chọn khu vực trước.")
                else:
                    with st.spinner("⏳ Đang quét ảnh từ GEE..."):
        ...
        with st.container(border=True):
            st.markdown("<div class='box-title'>3️⃣ Đa thời gian</div>", unsafe_allow_html=True)
        ...
        if run_btn:
            if not selected_provinces:
                st.warning("⚠️ Hãy chọn ít nhất 1 tỉnh/bang để tiếp tục.")
            else:
                ...
                with st.spinner("⏳ Đang phân tích dữ liệu GEE... Vui lòng chờ..."):
                    try:
                        ...
                        st.error(f"❌ Lỗi phân tích: {str(e)[:100]}")

        if st.session_state["analyzed"] and st.session_state["result"] is not None:
            result_years = st.session_state["result"]["years"]

            st.markdown("<div class='box-title'>4️⃣ Chế độ bản đồ</div>", unsafe_allow_html=True)
            ...
            if st.session_state["map_mode"] == "🎬 Xem từng năm":
                st.markdown("<div class='box-title'>5️⃣ Timeline năm</div>", unsafe_allow_html=True)
                ...
                st.caption(f"🎞️ Năm hiện tại: **{st.session_state['display_year']}**")
                ...
                st.markdown("<div class='box-title'>5️⃣ So sánh 2 năm</div>", unsafe_allow_html=True)
                ...
                st.caption(
                    f"🪞 So sánh {layer}: kéo giữa **{st.session_state['compare_left_year']}** và **{st.session_state['compare_right_year']}**"
                )
...
with col_map:
    if st.session_state["analyzed"] and st.session_state["result"] is not None:
        ...
        if st.session_state["map_mode"] == "🎬 Xem từng năm":
            ...
            st.markdown(
                ...
                <div class="map-banner-cinema">
                    🎬 XEM TỪNG NĂM — {params.layer} / NĂM {st.session_state["display_year"]}
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            st.markdown(
                ...
                <div class="map-banner-swipe">
                    🪞 SO SÁNH KÉO — {params.layer} / {st.session_state["compare_left_year"]} ↔ {st.session_state["compare_right_year"]}
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        if st.session_state["last_map_zoom"] is not None:
            meta_cols[0].caption(f"🔎 Mức zoom: {st.session_state['last_map_zoom']}")
        if st.session_state["last_map_bounds"] is not None:
            meta_cols[1].caption("🧭 Đã ghi nhận khung nhìn")
...
with col_report:
    with st.container(height=900, border=False):
        st.markdown("<div class='danger-title'>📊 BÁO CÁO CHI TIẾT</div>", unsafe_allow_html=True)
        ...
        if st.session_state["analyzed"] and st.session_state["result"] is not None:
            ...
            k1.markdown(
                f"<div class='kpi-card'><div class='kpi-title'>Trung bình</div><div class='kpi-value'>{format_pixel_value(params.layer, mean_val)}</div></div>",
                unsafe_allow_html=True
            )
            k2.markdown(
                f"<div class='kpi-card'><div class='kpi-title'>Tối thiểu</div><div class='kpi-value'>{format_pixel_value(params.layer, min_val)}</div></div>",
                unsafe_allow_html=True
            )
            k3.markdown(
                f"<div class='kpi-card'><div class='kpi-title'>Tối đa</div><div class='kpi-value'>{format_pixel_value(params.layer, max_val)}</div></div>",
                unsafe_allow_html=True
            )

            st.markdown("<div class='danger-title' style='font-size: 15px; color: #d32f2f;'>📈 BIỂU ĐỒ PHÂN TÍCH</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='chart-box'><b>2. Phân bố giá trị ({result['last_y']})</b>", unsafe_allow_html=True)
            ...
            if hist and "histogram" in hist and len(hist["histogram"]) > 0:
                ...
                fig2.update_layout(margin=dict(l=0, r=0, t=10, b=0), height=220, hovermode='x')
            ...
            if len(result["years"]) > 1:
                st.markdown(f"<div class='chart-box'><b>4. Tương quan ({result['first_y']} vs {result['last_y']})</b>", unsafe_allow_html=True)
                if result["scatter_df"] is not None and not result["scatter_df"].empty:
                    ...
                    fig4.update_layout(margin=dict(l=0, r=0, t=10, b=0), height=220, hovermode='closest')
            ...
            if len(params.provinces) > 1:
                st.markdown("<div class='chart-box'><b>5. So sánh khu vực</b>", unsafe_allow_html=True)
                if result["bar_data"]:
                    fig5 = px.bar(pd.DataFrame(result["bar_data"]), x="Khu vực", y="Trị số", text_auto=".3f")
                    fig5.update_traces(marker_color="#00a86b")
                    fig5.update_layout(margin=dict(l=0, r=0, t=10, b=0), height=260, hovermode='x')
                    st.plotly_chart(fig5, use_container_width=True, config={"displayModeBar": False})
                else:
                    st.warning("Không có dữ liệu khu vực.")
                st.markdown("</div>", unsafe_allow_html=True)
        else:
            st.info("👈 Chọn khu vực và bấm PHÂN TÍCH HỆ THỐNG để bắt đầu.")
