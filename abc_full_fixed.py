...
# =========================================================
# 2.1. BẢO MẬT API KEY GEMINI
# =========================================================
def get_gemini_api_key() -> str:
    """
    Ưu tiên đọc API key từ Streamlit Secrets.
    Nếu không có, fallback sang biến môi trường GEMINI_API_KEY.

    Cách cấu hình khuyến nghị:
    1) Tạo thư mục .streamlit
    ...
    if not api_key:
        raise ValueError(
            ...
            "Hãy thêm key vào .streamlit/secrets.toml hoặc biến môi trường GEMINI_API_KEY."
        )
    ...
    with st.container(border=False):
        ...
        if st.session_state["analyzed"] and st.session_state["result"] is not None:
            ...
            def generate_gemini_report(layer, first_year, last_year, first_val, last_val, high_area, roi_name):
                """
                Sinh báo cáo XAI bằng Gemini.
                AI chỉ được phép diễn giải dựa trên dữ liệu thật đã tính từ GEE:
                layer, first_val, last_val, high_area, roi_name.
                """
                try:
                    ...
                    prompt = f"""
                    Bạn là chuyên gia GIS và quy hoạch đô thị.

                    Hãy viết 1 đoạn báo cáo học thuật khoảng 150 chữ bằng tiếng Việt
                    về khu vực {roi_name} trong giai đoạn {first_year}-{last_year}.

                    Dữ liệu thực tế từ Google Earth Engine:
                    - Chỉ số phân tích: {layer}
                    - Giá trị đầu kỳ {first_year}: {first_val:.3f}
                    - Giá trị cuối kỳ {last_year}: {last_val:.3f}
                    - Mức thay đổi tuyệt đối: {delta:.3f}
                    - Xu hướng: {direction}
                    - Diện tích vùng rủi ro/cấp cao: {high_area:,.0f} ha

                    Yêu cầu:
                    - Không bịa thêm số liệu.
                    - Không suy diễn vượt quá dữ liệu đã cung cấp.
                    - Giải thích ý nghĩa quy hoạch dựa trên chỉ số {layer}.
                    - Có cảnh báo môi trường và đề xuất định hướng vĩ mô.
